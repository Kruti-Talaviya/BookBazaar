...
exports.purchaseBook = async (req, res) => {
  try {
    ...
    console.log(`BookId: ${bookId}, BuyerId: ${buyerId}`);
    const book = await Book.findById(bookId);
    if (!book) {
      return res.status(404).json({ message: 'Book not found' });
    }
...
exports.findSoldBooksBySellerId = async (req, res) => {
  try {
    const { sellerId } = req.params;

    const soldBooks = await Book.find({
      seller: sellerId,
      buyer: { $ne: null } // Only books that have a buyer
    })
    .populate({
      path: 'seller',
      populate: {
        path: 'user', // Assuming `user` is within `seller`
        select: 'username email' // Selecting specific user fields
      }
    })
    .exec();
    ...
    if (soldBooks.length > 0) {
      return res.status(200).json({
        status: true,
        message: 'Sold books found',
        books: soldBooks
      });
    } else {
      return res.status(404).json({
        status: false,
        message: 'No sold books found for this seller',
      });
    }
  } catch (error) {
    ...
    return res.status(500).json({
      status: false,
      message: 'An error occurred while fetching sold books',
      error: error.message,
    });
  }
};
